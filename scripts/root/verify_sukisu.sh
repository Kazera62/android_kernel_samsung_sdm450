...
grep -Fq 'typedef long (*syscall_fn_t)(const struct pt_regs *);' "$KSU_DIR/hook/syscall_hook.h" || die "Linux 4.9 ARM64 syscall_fn_t typedef missing"
...
test -f "$KSU_DIR/selinux/sepolicy.c" || die "SukiSU sepolicy.c missing"
grep -Fq 'flex_array_get_ptr((a)->htable' "$KSU_DIR/selinux/sepolicy.c" || die "Linux 4.9 sepolicy AVTAB flex_array adapter missing"
...
grep -Fq 'return __vmalloc(size, flags, PAGE_KERNEL);' "$KSU_DIR/kernel_compat.h" || die "Linux 4.9 kvmalloc fallback missing"
grep -Fq 'vfree(addr);' "$KSU_DIR/kernel_compat.h" || die "Linux 4.9 kvfree fallback missing"
...
grep -Fq '#include <asm/pgtable.h>' "$KSU_DIR/kernel_compat.h" || die "Linux 4.9 VMALLOC range header missing"
if grep -RqsE '(^|[^A-Za-z0-9_])kvmalloc\(' "$KSU_DIR" --exclude=kernel_compat.h; then
  die "raw kvmalloc() call remains outside kernel_compat.h"
fi
if grep -RqsE '(^|[^A-Za-z0-9_])kvfree\(' "$KSU_DIR" --exclude=kernel_compat.h; then
  die "raw kvfree() call remains outside kernel_compat.h"
fi
