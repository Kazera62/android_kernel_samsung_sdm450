...
rm -rf "$KSU_DIR"
git clone --depth=1 --branch "$SUKISU_VERSION" "$SUKISU_REPO" "$KSU_DIR"
git -C "$KSU_DIR" fetch --depth=1 origin "refs/tags/$SUKISU_VERSION:refs/tags/$SUKISU_VERSION"
git -C "$KSU_DIR" checkout --detach "$SUKISU_VERSION"
